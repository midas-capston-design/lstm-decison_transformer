#!/usr/bin/env python3
"""
지자기 기반 실내 위치 추정 - 데이터 전처리 v4
- 그리드 크기 확대 (0.45m → 0.9m)
- 클래스 수 대폭 감소로 학습 안정성 개선
"""
import pandas as pd
import numpy as np
import os
...
output_dir = Path('processed_data_v4')
...
📊 최종 데이터셋:
  ...
  클래스: {len(valid_classes):,}개 ⭐ (v3의 ~2,152개 대비 약 1/4로 감소!)
...
🎯 v4 핵심 변경점:
  ⭐ 그리드 크기 확대 (0.45m → 0.9m): 클래스 수 약 1/4로 감소
  ✅ 클래스당 샘플 수 대폭 증가 (39개 → 약 156개)
  ✅ 오버피팅 문제 해결 기대
  ✅ 학습 안정성 및 일반화 성능 향상
