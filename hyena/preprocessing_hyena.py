...
# 증강 설정 (Train only) - Smooth augmentation
AUGMENT_RATIO = 0.5  # Train 데이터의 50%에 증강 적용
MAG_NOISE_STD = 0.3    # 지자기 smooth noise 표준편차 (μT)
ORIENT_NOISE_STD = 0.5  # 방향 smooth noise 표준편차 (도)
SMOOTH_SIGMA = 3.0     # Gaussian smoothing sigma

print("="*70)
print("🔧 Hyena용 데이터 전처리")
print("="*70)
print(f"  Window: {WINDOW_SIZE} 샘플 (5초 @ 50Hz)")
print(f"  Stride: {STRIDE} 샘플 (1초)")
...
def augment_sequence_sequential(seq):
    """
    Smooth Augmentation: 시간적으로 연속적인 노이즈

    Gaussian smoothing을 적용한 부드러운 노이즈 추가
    → 시퀀셜 패턴 유지하면서 자연스러운 변화

    Args:
        seq: (250, 6) numpy array [MagX, MagY, MagZ, Pitch, Roll, Yaw]
    ...
    for i in range(3):
        noise_mag[:, i] = gaussian_filter1d(noise_mag[:, i], sigma=SMOOTH_SIGMA)
        noise_orient[:, i] = gaussian_filter1d(noise_orient[:, i], sigma=SMOOTH_SIGMA)
...
print("\n[4/8] Train/Val/Test 분할 (위치 커버리지 보장)...")
...
for data in all_data:
    ...
    for coord in data['coords']:
        all_positions.append(coord)

all_positions = np.array(all_positions)
n_total = len(all_positions)

print(f"  총 {n_total}개의 250-샘플 덩어리")

# 2. 그리드 셀별로 샘플 그룹화
grid_to_indices = defaultdict(list)
for idx, (x, y) in enumerate(all_positions):
    grid_id = coord_to_grid(x, y)
    grid_to_indices[grid_id].append(idx)

n_grids = len(grid_to_indices)
print(f"  {n_grids}개 그리드 셀에 분포")

# 3. 각 그리드 셀마다 최소 1개 샘플을 Train에 배정
np.random.seed(42)
train_indices = []
covered_grids = set()

for grid_id, indices in grid_to_indices.items():
    # 각 그리드에서 최소 1개 선택
    np.random.shuffle(indices)
    n_train_per_grid = max(1, int(len(indices) * 0.7))  # 70% 또는 최소 1개
    train_indices.extend(indices[:n_train_per_grid])
    covered_grids.add(grid_id)

print(f"  모든 {len(covered_grids)}개 그리드 셀이 Train에 포함")
...
# Val/Test 분할 (50:50)
n_val = len(remaining_indices) // 2
val_indices = remaining_indices[:n_val]
test_indices = remaining_indices[n_val:]

print(f"  Train: {len(train_indices)}개 ({len(train_indices)/n_total*100:.1f}%)")
print(f"  Val:   {len(val_indices)}개 ({len(val_indices)/n_total*100:.1f}%)")
print(f"  Test:  {len(test_indices)}개 ({len(test_indices)/n_total*100:.1f}%)")
...
output_dir = Path(__file__).parent / 'processed_data_hyena'
...
np.save(output_dir / 'positions_train.npy', coords_train_final)  # Hyena용
...
np.save(output_dir / 'positions_val.npy', coords_val)  # Hyena용
...
np.save(output_dir / 'positions_test.npy', coords_test)  # Hyena용
