#!/usr/bin/env python3
"""
지자기 기반 실내 위치 추정 - 데이터 전처리 v4 + Fuzzy
- 그리드 크기: 0.9m
- Fuzzy normalization (Low/Medium/High)
"""
import pandas as pd
import numpy as np
import os
...
# 설정
WINDOW_SIZE = 250  # 샘플 (5초, ~5걸음)
STRIDE = 5         # Sliding window stride (5샘플 = 0.1초 간격)
GRID_SIZE = 0.9    # m (v3의 0.45m에서 2배 확대)
...
print("="*70)
print("🔧 데이터 전처리 v4 + Fuzzy 시작 (그리드 0.9m)")
print("="*70)
print(f"  Window: {WINDOW_SIZE} 샘플 (5초, ~5걸음)")
print(f"  Stride: {STRIDE} 샘플 (0.1초)")
print(f"  Grid: {GRID_SIZE}m")
print(f"  정규화: Fuzzy (Low/Medium/High)")
...
def fuzzy_normalize_feature(data):
    """
    단일 특징에 대해 Low/Medium/High 멤버십 계산

    Args:
        data: (n_samples,) numpy array

    Returns:
        fuzzy_data: (n_samples, 3) - [low, medium, high]
    """
    ...
    fuzzy_data = np.zeros((len(data), 3))

    for i, x in enumerate(data):
        # Low: min → q25 → q50
        fuzzy_data[i, 0] = triangular_membership(x, min_val, min_val, q50)
        # Medium: q25 → q50 → q75
        fuzzy_data[i, 1] = triangular_membership(x, q25, q50, q75)
        # High: q50 → max → max
        fuzzy_data[i, 2] = triangular_membership(x, q50, max_val, max_val)
...
# Fuzzy 정규화
print(f"\n  Fuzzy 정규화 적용 중...")
n_samples, n_timesteps, n_features = all_sequences.shape

# (n_samples * n_timesteps, n_features) 형태로 reshape
all_sequences_flat = all_sequences.reshape(-1, n_features)

# 각 특징에 대해 fuzzy 정규화
fuzzy_features = []
for i in range(n_features):
    feature_data = all_sequences_flat[:, i]
    fuzzy_feat = fuzzy_normalize_feature(feature_data)
    fuzzy_features.append(fuzzy_feat)

# (n_samples * n_timesteps, n_features * 3) → (n_samples, n_timesteps, n_features * 3)
all_sequences_fuzzy = np.hstack(fuzzy_features)
all_sequences_norm = all_sequences_fuzzy.reshape(n_samples, n_timesteps, n_features * 3)

print(f"  Fuzzy 정규화 완료: {all_sequences.shape} → {all_sequences_norm.shape}")
print(f"  입력 차원: 6 features → 18 features (각 feature × 3 fuzzy values)")
...
output_dir = Path('processed_data_v4_fuzzy')
...
🎯 v4 + Fuzzy 변경점:
  - 그리드 크기: 0.9m
  - Fuzzy 정규화: 6 features → 18 features (Low/Medium/High)
  - 노이즈에 강건한 표현
