#!/usr/bin/env python3
"""
Decision Transformer용 데이터 전처리 (개선 버전)
- 7:2:1 split
- 지자기 데이터 증강 (1-3uT 노이즈)
"""
import pandas as pd
import numpy as np
...
print("=" * 70)
print("🚀 Decision Transformer용 데이터 전처리 (7:2:1 + 증강)")
print("=" * 70)
...
AUGMENT_NOISE_RANGE = (1.0, 3.0)  # uT

print(f"  Window: {WINDOW_SIZE} 샘플")
print(f"  Stride: {STRIDE} 샘플")
print(f"  지자기 노이즈: {AUGMENT_NOISE_RANGE[0]}-{AUGMENT_NOISE_RANGE[1]} uT")
...
# ============================================================================
# 6단계: Train/Val/Test 분할 (7:2:1)
# ============================================================================

print("\n[4/8] Train/Val/Test 분할 (7:2:1)...")
...
n_val = int(0.2 * n_routes)

train_routes = routes[:n_train]
val_routes = routes[n_train:n_train+n_val]
test_routes = routes[n_train+n_val:]

print(f"  Train 경로: {len(train_routes)} ({len(train_routes)/n_routes*100:.1f}%)")
print(f"  Val 경로: {len(val_routes)} ({len(val_routes)/n_routes*100:.1f}%)")
print(f"  Test 경로: {len(test_routes)} ({len(test_routes)/n_routes*100:.1f}%)")
...
for seq, traj in tqdm(zip(train_sequences_orig, train_trajectories_orig),
    ...
    # 증강 (지자기 노이즈)
    seq_aug = augment_magnetic_data(seq, AUGMENT_NOISE_RANGE)
...
output_dir = Path('processed_data_dt_v2')
...
metadata = {
    ...
    'augment_noise_range': AUGMENT_NOISE_RANGE,
...
🔊 데이터 증강:
  지자기 노이즈: {AUGMENT_NOISE_RANGE[0]}-{AUGMENT_NOISE_RANGE[1]} uT
